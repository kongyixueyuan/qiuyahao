...
// 生成新的钱包
// 从数据库中读取，如果不存在
func QYH_NewWallets() (*QYH_Wallets, error) {
	wallets := QYH_Wallets{}
	wallets.QYH_Wallets = make(map[string]*QYH_Wallet)

	err := wallets.QYH_LoadFromFile()
...
// 从数据库中读取钱包列表
func (ws *QYH_Wallets) QYH_LoadFromFile() error {
...
// 将钱包存到数据库中
func (ws *QYH_Wallets) QYH_SaveToFile() {
...
// 打印所有钱包的余额
func (ws *QYH_Wallets) QYH_GetBalanceAll() map[string]int {
