...
func QYH_startServer(nodeID string,minerAdd string)  {
	...
	bc := QYH_BlockchainObject(nodeID)
	...
	if nodeAddress != knowNodes[0]{
		...
		QYH_sendVersion(knowNodes[0],bc)
	}
	...
	for {
		...
		go QYH_handleConnection(conn,bc)
...
func QYH_handleConnection(conn net.Conn,bc *QYH_Blockchain) {
